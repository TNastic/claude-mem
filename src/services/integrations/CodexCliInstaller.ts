 ...
 * 3. Injects context via each project's AGENTS.md (Codex reads this natively)
 *
 * Anti-patterns:
 ...
 * 2. Project AGENTS.md files are updated from each Codex session cwd
 *
...
export async function installCodexCli(): Promise<number> {
  ...
  try {
    ...
    console.log(`  Context path: <project>/AGENTS.md (resolved from Codex session cwd)`);

    console.log(`
Installation complete!

Transcript watch config: ${DEFAULT_CONFIG_PATH}
Context files: <project>/AGENTS.md

How it works:
  - claude-mem watches Codex session JSONL files for new activity
  - No hooks needed -- transcript watching is fully automatic
  - Context from past sessions is injected via each project's AGENTS.md
 ...
 * 2. Leaves project AGENTS.md files untouched (removes watcher config only)
 *
 * @returns 0 on success, 1 on failure
 */
...
export function checkCodexCliStatus(): number {
  ...
  try {
    ...
    if (codexWatch.context) {
      ...
      console.log(`  Context path: ${codexWatch.context.path ?? '<project>/AGENTS.md'}`);
